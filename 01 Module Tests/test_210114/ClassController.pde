...
class Controller {
  ...
  boolean mode = false; // false = 600, volle länge. / true = akzeptiert cm
  float pixelpitch = 1.6666666667;
  ...
  void setLength(int segmentNr, float x1, float y1, float z1, float x2, float y2, float z2, int cm) {
    if(segmentNr >= 0 && segmentNr <= 1) {
      ports[segmentNr].setLength(x1, y1, z1, x2, y2, z2, cm);
      //println("setting coordinates["+segmentNr+"]= (" + x1 + " / " + y1 + " / " + z1 + "), (" + x2 + " / " + y2 + " / " + z2 +")");
    } else println("error= segmentNr " + segmentNr + " doesn't exist");
    mode = true;
  }
  ...
  class Port {
    ...
    void setLength(float x1, float y1, float z1, float x2, float y2, float z2, int cm) {
      segment.setPos(x1, y1, z1, x2, y2, z2, cm);
    }
    ...
    class Segment {
      ...
      float segmentLength = LEDperSegment*pixelpitch;
      ...
      void setPos(float x1, float y1, float z1, float x2, float y2, float z2, int cm) {
        println(">> >> setting coordinates["+nr+"]= (" + x1 + " / " + y1 + " / " + z1 + "), (" + x2 + " / " + y2 + " / " + z2 +")");
        startPos = new PVector(x1, y1, z1);
        endPos = new PVector(x2, y2, z2);
      }
